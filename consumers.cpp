...
void *Consumer(void *arg){
    ...
    while(broker->consumed < broker->maxRides){ //Checks to see if consumed is not equal to maxrides
        ...
        if(broker->consumed < broker->maxRides){
            ...
            //Print statements
            if(requester == CostAlgoDispatch){ //checks if requester was cost algo
                int costSave[RequestTypeN] = {broker->consumedTotals[requester][CostAlgoDispatch], broker->consumedTotals[requester][FastAlgoDispatch]};
                ...
                else if(requester == FastAlgoDispatch){ //checks if requester is fast algo
                    ...
                    if(requestID == HumanDriver){ //Checks if requestID was a human
                        sem_post(&broker->maxHumanDrivers); //If requestID was human, frees up space for human requestID from max of 4
                        io_remove_type(FastAlgoDispatch, HumanDriver, broker->inRequestQueue, fastRide);
                    }
                    if(requestID == RoboDriver){ //checks if requestID was a robo
